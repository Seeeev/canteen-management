...
const { fetchRequests, approveRequest, rejectRequest } = useWithdrawalRequests()
...
async function onApprove(row: any) {
  ...
  useToast().add({
    title: 'Approved',
    description: `Request #${row.id} approved`,
    color: 'success',
  })
  loadTable()
}
...
async function onReject(row: any) {
  const notes = prompt(`Reject request #${row.id}. Notes:`)
  if (notes === null) return
  await rejectRequest(row.id, notes)
  useToast().add({
    title: 'Rejected',
    description: `Request #${row.id} rejected`,
    color: 'warning',
  })
  loadTable()
}
